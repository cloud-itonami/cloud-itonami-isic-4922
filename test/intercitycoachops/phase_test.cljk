(ns intercitycoachops.phase-test
  "Unit tests of `intercitycoachops.phase` rollout logic."
...
(deftest phase-1-service-record-only
  (testing "phase 1 allows only service-record logging, requires approval"
    ...
    (let [{:keys [disposition]} (phase/gate 1 {:op :schedule-dispatch-operation} :commit)]
      (is (= :hold disposition)))))
...
(deftest phase-3-auto-commits-clean-ops
  (testing "phase 3 auto-commits clean, high-conf non-safety ops"
    ...
    (let [{:keys [disposition]} (phase/gate 3 {:op :schedule-dispatch-operation} :commit)]
      (is (= :commit disposition)))
