(ns intercitycoachops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (IntercityCoachAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the IntercityCoachGovernor (:govern)
  ...
  human dispatch/fleet coordinator. The approver resumes with
  `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [intercitycoachops.advisor :as advisor]
            [intercitycoachops.governor :as governor]
            [intercitycoachops.phase :as phase]
...
(defn build
  ...
  `intercitycoachops.store/Store`).
  opts:
    :advisor      -- an `intercitycoachops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; IntercityCoachAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; IntercityCoachGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :vehicle-id (:vehicle-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :vehicle-id (:vehicle-id request) :by (:by approval)}]}
            {:disposition :hold
